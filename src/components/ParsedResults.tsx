...
export const ParsedResults: React.FC<ParsedResultsProps> = ({ data, isLoading }) => {
  ...
  const renderConfidenceBadge = (score?: number) => {
    ...
    return (
      <span className={`inline-flex items-center rounded-md border px-1.5 py-0.5 text-[9px] font-semibold ${colorClass} select-none flex-shrink-0`}>
        {score}% Confidence
      </span>
    );
  };
  ...
  const renderPersonalInfo = () => (
    <Card className="p-6 border border-slate-200 bg-white shadow-sm space-y-5 h-full flex flex-col justify-between">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-100 w-full">
          <div className="flex items-center space-x-2.5">
            <div className="p-2 bg-indigo-50 text-[#4f46e5] rounded-lg flex-shrink-0">
        ...
        <div className="space-y-3.5 text-xs">
          <div className="grid grid-cols-3 gap-2">
            ...
            <span className="text-slate-800 font-semibold col-span-2 break-words">{data.name || 'Not Specified'}</span>
          </div>
          <div className="grid grid-cols-3 gap-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <span className="text-slate-800 font-semibold col-span-2 break-words">{data.phone || 'Not Specified'}</span>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <span className="text-slate-400 font-medium">Location</span>
            <span className="text-slate-800 font-semibold col-span-2 break-words">{locationValue}</span>
          </div>
        </div>
      </div>
    </Card>
              ...
              return (
                <div key={idx} className="relative group text-left">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <h4 className="text-xs font-semibold text-slate-800 break-words flex-1 min-w-0">{title}</h4>
                    {dateString && (
                      <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap flex-shrink-0">{dateString}</span>
                    )}
                  </div>
                  {company && <p className="text-[11px] text-slate-500 font-medium mt-0.5 break-words">{company}</p>}
                  {descLines.length > 0 ? (
                    <p className="text-[11px] text-slate-500 leading-relaxed mt-2 whitespace-pre-line break-words">{descLines.join('\n')}</p>
                  ) : null}
                </div>
              );
            })}
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.skills.map((skill, idx) => (
              <Badge 
                key={idx} 
                variant="secondary" 
                className="text-[10px] px-2.5 py-1 font-medium bg-[#f1f5f9] text-slate-700 hover:bg-slate-200 border-0 rounded-md break-all"
              >
                {skill}
              </Badge>
            ))}
            ...
            {data.education.map((edu, idx) => {
              ...
              return (
                <div key={idx} className="space-y-1 text-left">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <h4 className="text-xs font-semibold text-slate-800 break-words flex-1 min-w-0">{degree}</h4>
                    {dateString && (
                      <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap flex-shrink-0">{dateString}</span>
                    )}
                  </div>
                  {school && <p className="text-[11px] text-slate-500 font-medium break-words">{school}</p>}
                </div>
              );
            })}
          </div>
        ...
        {data.projects && data.projects.length > 0 ? (
          <div className="space-y-4">
            {data.projects.map((proj, idx) => {
              ...
              return (
                <div key={idx} className="space-y-1 text-left">
                  <h4 className="text-xs font-semibold text-slate-800 break-words">{title}</h4>
                  {desc && <p className="text-[11px] text-slate-500 leading-relaxed break-words">{desc}</p>}
                </div>
              );
            })}
          </div>
    ...
    <Card className="p-6 border border-slate-200 bg-white shadow-sm space-y-5 h-full flex flex-col justify-between">
      <div className="space-y-4">
        ...
        {data.certifications && data.certifications.length > 0 ? (
          <ul className="space-y-2 text-xs text-slate-600 list-disc pl-4 text-left font-medium break-words">
  ...
  const renderLinks = () => (
    <Card className="p-6 border border-slate-200 bg-white shadow-sm space-y-5 h-full flex flex-col justify-between">
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-100 w-full">
          <div className="flex items-center space-x-2.5">
            <div className="p-2 bg-pink-50 text-pink-600 rounded-lg flex-shrink-0">
            ...
            {data.links.map((link, idx) => {
              ...
              return (
                <a
                  ...
                  className="flex items-center justify-between text-xs text-slate-600 hover:text-slate-800 border border-slate-100 bg-slate-50/50 hover:bg-slate-100 p-2.5 rounded-md transition-colors min-w-0 w-full"
                >
                  ...
                  <ExternalLink className="h-3 w-3 text-slate-400 flex-shrink-0 ml-2" />
                </a>
              );
            })}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-4 bg-slate-50/30 p-4 rounded-xl border border-slate-150">
        <div className="flex items-center space-x-2.5">
          ...
          <span className="inline-flex items-center rounded bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-600 ring-1 ring-inset ring-emerald-500/10 select-none">
            Success
          </span>
        </div>
      ...
      <div className="border-b border-slate-200 overflow-x-auto scrollbar-none">
        <nav className="flex space-x-8 -mb-px min-w-max pb-1" aria-label="Tabs">
          {TABS.map((tab) => {
